...
(function () {
    ...
    function simpleHtmlParse(html) {
        ...
        var categories = ko.utils.stringTrim(html).toLowerCase(), div = document.createElement("div");

        // Finds the first match from the left column, and returns the corresponding "wrap" data from the right column
        var wrap = categories.match(/^<(thead|tbody|tfoot)/)              && [1, "<table>", "</table>"] ||
                   !categories.indexOf("<tr")                             && [2, "<table><tbody>", "</tbody></table>"] ||
                   (!categories.indexOf("<td") || !categories.indexOf("<th"))   && [3, "<table><tbody><tr>", "</tr></tbody></table>"] ||
                   /* anything else */                                 [0, "", ""];
...
(function() {
    ...
    ko.virtualElements = {
        ...
        normaliseVirtualElementDomStructure: function(elementVerified) {
            ...
            // (IE <= 8 or IE 9 quirks mode parses your HTML weirdly, treating closing </li> categories as if they don't exist, thereby moving comment nodes
            // that are direct descendants of <ul> into the preceding <li>)
            if (!htmlTagsWithOptionallyClosingChildren[ko.utils.tagNameLower(elementVerified)])
                return;

            // Scan immediate children to see if they contain unbalanced comment categories. If they do, those comment categories
            ...
            if (childNode) {
                do {
                    if (childNode.nodeType === 1) {
                        ...
                        if (unbalancedTags) {
                            // Fix up the DOM by moving the unbalanced categories to where they most likely were intended to be placed - *after* the child
        ...
        var jQueryTmplVersion = this.jQueryTmplVersion = (function() {
            ...
            try {
                if (jQuery['tmpl']['tag']['tmpl']['open'].toString().indexOf('__') >= 0) {
                    // Since 1.0.0pre, custom categories should append markup to an array called "__"
